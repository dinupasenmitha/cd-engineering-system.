...
APP.Invoices = (function () {
  ...
  async function renderDetail(id) {
    ...
    const jobParts = job ? await S().getJobParts(job.id) : [];
    const bc = inv.status === 'Paid' ? 'badge-paid' : 'badge-unpaid';
    const main = document.getElementById('main-content');
    ...
    main.innerHTML = `
      <div class="fade-in">
        ...
        <div class="btn-group mb-20">
          ...
          ${job ? `<button class="btn btn-outline" onclick="APP.Router.navigate('documents','upload','${job.id}')">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="18" height="18"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg> Upload Signed Doc
          </button>` : ''}
        ...
        <div class="invoice-print" id="invoicePrint">
          ...
          <table class="data-table invoice-table">
            ...
            <tbody>
              ${jobParts.length ? jobParts.map((jp, i) => `<tr><td style="text-align:center">${i + 1}</td><td>${jp.name}</td><td style="text-align:center">${jp.quantity}</td><td style="text-align:right">${S().formatCurrency(jp.unit_price)}</td><td style="text-align:right">${S().formatCurrency(jp.quantity * jp.unit_price)}</td></tr>`).join('') : `<tr><td colspan="5" style="text-align:center;color:var(--gray-400);padding:16px">Parts/Materials included in service</td></tr>`}
              <tr><td></td><td><strong>Labor Charges</strong></td><td style="text-align:center">1</td><td style="text-align:right">${S().formatCurrency(inv.laborCost)}</td><td style="text-align:right">${S().formatCurrency(inv.laborCost)}</td></tr>
              <tr><td></td><td><strong>Transport / Logistics</strong></td><td style="text-align:center">1</td><td style="text-align:right">${S().formatCurrency(inv.transportCost)}</td><td style="text-align:right">${S().formatCurrency(inv.transportCost)}</td></tr>
            </tbody>
          </table>
